...
const TicketDetail = ({ ticket, onTicketUpdated }) => {
  ...
  const getStatusBadge = (status) => {
    switch (status) {
      case 'OPEN':
      case 'IN_PROGRESS':
        return <span className="px-2 py-1 rounded bg-amber-50 text-amber-700 text-xs font-bold border border-amber-200">Pending</span>;
      case 'RESOLVED':
        return <span className="px-2 py-1 rounded bg-emerald-50 text-emerald-700 text-xs font-bold border border-emerald-200">Resolved</span>;
      case 'DISMISSED':
        return <span className="px-2 py-1 rounded bg-gray-100 text-gray-700 text-xs font-bold border border-gray-200">Dismissed</span>;
      default:
          ...
          <div className="flex justify-between items-center mb-3">
            ...
            {ticket.ticketType === 'REFUND' && ticket.requestedAmount && (
              <p className="text-2xl text-red-600 font-bold whitespace-nowrap">
                {Number(ticket.requestedAmount).toLocaleString('en-US')} ₫
              </p>
            )}
          </div>
        ...
        <div>
          ...
          <div className="relative">
            <textarea
              ...
              placeholder="Enter official response for the student..."
              value={adminResponse}
      ...
      {ticket.status !== 'RESOLVED' && ticket.status !== 'DISMISSED' && (
        <div className="absolute bottom-0 left-0 right-0 p-6 bg-white/95 backdrop-blur-md border-t border-gray-200 flex justify-between items-center shadow-[0_-10px_40px_rgba(0,0,0,0.03)] z-10">
          <div className="text-gray-600 text-sm flex items-center gap-2">
            ...
            <span className="font-medium">Please double check before making a decision</span>
          </div>
          <div className="flex gap-4">
            ...
            >
              {ticket.ticketType === 'REFUND' ? 'Approve Refund' : 'Mark as Resolved'}
            </button>
          </div>
        </div>
      )}
